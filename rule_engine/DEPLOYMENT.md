...
2. **EagleSoft SSL (required for Rule Engine ↔ EagleSoft)**  
   ...
   **Temporary / test (same as local – no certs):** In `docker-compose.yml`, `ES_SSL_CLIENT_TRUSTALL=true` is set so the backend accepts any EagleSoft server certificate. No `.env` and no cert setup needed; you can remove or leave empty the `certs/` directory on the server. For production later, set `ES_SSL_CLIENT_TRUSTALL=false` in the compose file and add proper `cacerts.jks`/`keystore.jks` as in Option A/B above.
