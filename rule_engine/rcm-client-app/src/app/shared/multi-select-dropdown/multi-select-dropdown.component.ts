...
export class MultiSelectDropdownComponent {
  ...
  isAllSelected: any = { 'clients': false, 'offices': false, 'ageCategory': false, 'claimStatus': false, 'teams': false, 'pendencyClients': true };

  claimDetailPageConfig: any = { 'serviceCodes': [], 'serviceCodesWithTooth': [], 'requiredRequestedReBilling': [], 'requiredRequestedReBillingWithTooth': [], 'requiredReBilling': [], 'requiredReBillingWithTooth': [], 'reBillingServiceCodes': [], 'reBillingServiceCodesWithTooth': [], recreateServiceCodes: [], recreateServiceCodesWithTooth: [] };
  pendencyTeamConfig: any = {
    'pendencyClients': []
  };
  ...
  ngAfterViewInit() {
    ...
    } else if (this.inputConfig?.isPendencyTeam && this.inputConfig?.pendencyClients && this.inputConfig?.pendencyClients?.length == 1) {
  ...
  getSelectedValue(status: Boolean, value: any, type: String, filterProperty?: string) {
    ...
    else if (type === 'serviceCodesWithTooth') {
      if (status) {
        let isExist = this.claimDetailPageConfig.serviceCodesWithTooth.some((ele: any) => ele.name.toUpperCase() == value.name.toUpperCase() && ele.tooth.toUpperCase() == value.tooth.toUpperCase() && ele.checked == value.checked);
        if (!isExist) {
          this.claimDetailPageConfig.serviceCodesWithTooth.push(value);
          this.emitToParent.emit({ action: 'updateServiceCodeWithTooth', value: this.claimDetailPageConfig.serviceCodesWithTooth });
        }
      } else {
        this.claimDetailPageConfig.serviceCodesWithTooth.forEach((e: any, idx: any) => {
          if (e.name == value.name) {
            this.claimDetailPageConfig.serviceCodesWithTooth.splice(idx, 1);
            this.emitToParent.emit({ action: 'updateServiceCodeWithTooth', value: this.claimDetailPageConfig.serviceCodesWithTooth });
          }
        });
    ...
    else if (type === 'reBillingServiceCodesWithTooth') {
      if (status) {
        let isExist = this.claimDetailPageConfig.reBillingServiceCodesWithTooth.some((ele: any) => ele.name.toUpperCase() == value.name.toUpperCase() && ele.tooth.toUpperCase() == value.tooth.toUpperCase() && ele.checked == value.checked);
        if (!isExist) {
          this.claimDetailPageConfig.reBillingServiceCodesWithTooth.push(value);
          this.emitToParent.emit({ action: 'updateReBillingServiceCodeWithTooth', value: this.claimDetailPageConfig.reBillingServiceCodesWithTooth });
        }
      } else {
        this.claimDetailPageConfig.reBillingServiceCodesWithTooth.forEach((e: any, idx: any) => {
          if (e.name == value.name) {
            this.claimDetailPageConfig.reBillingServiceCodesWithTooth.splice(idx, 1);
            this.emitToParent.emit({ action: 'updateReBillingServiceCodeWithTooth', value: this.claimDetailPageConfig.reBillingServiceCodesWithTooth });
          }
        });
    ...
    else if (type === 'requiredRequestedReBillingWithTooth') {
      if (status) {
        let isExist = this.claimDetailPageConfig.requiredRequestedReBillingWithTooth.some((ele: any) => ele.name.toUpperCase() == value.name.toUpperCase() && ele.tooth.toUpperCase() == value.tooth.toUpperCase() && ele.checked == value.checked);
        if (!isExist) {
          this.claimDetailPageConfig.requiredRequestedReBillingWithTooth.push(value);
          this.emitToParent.emit({ action: 'updateRequiredReBillingWithTooth', value: this.claimDetailPageConfig.requiredRequestedReBillingWithTooth });
        }
      } else {
        this.claimDetailPageConfig.requiredRequestedReBillingWithTooth.forEach((e: any, idx: any) => {
          if (e.name == value.name) {
            this.claimDetailPageConfig.requiredRequestedReBillingWithTooth.splice(idx, 1);
            this.emitToParent.emit({ action: 'updateRequiredReBillingWithTooth', value: this.claimDetailPageConfig.requiredRequestedReBillingWithTooth });
          }
        });
    ...
    else if (type === 'requiredReBillingWithTooth') {
      if (status) {
        let isExist = this.claimDetailPageConfig.requiredReBillingWithTooth.some((ele: any) => ele.name.toUpperCase() == value.name.toUpperCase() && ele.tooth.toUpperCase() == value.tooth.toUpperCase() && ele.checked == value.checked);
        if (!isExist) {
          this.claimDetailPageConfig.requiredReBillingWithTooth.push(value);
          this.emitToParent.emit({ action: 'updateReBillingWithTooth', value: this.claimDetailPageConfig.requiredReBillingWithTooth });
        }
      } else {
        this.claimDetailPageConfig.requiredReBillingWithToot.forEach((e: any, idx: any) => {
          if (e.name == value.name) {
            this.claimDetailPageConfig.requiredReBillingWithToot.splice(idx, 1);
            this.emitToParent.emit({ action: 'updateReBillingWithTooth', value: this.claimDetailPageConfig.requiredReBillingWithTooth });
          }
        });
    ...
    else if (type === 'recreateServiceCodesWithTooth') {
      if (status) {
        let isExist = this.claimDetailPageConfig.recreateServiceCodesWithTooth.some((ele: any) => ele.name.toUpperCase() == value.name.toUpperCase() && ele.tooth.toUpperCase() == value.tooth.toUpperCase() && ele.checked == value.checked);
        if (!isExist) {
          this.claimDetailPageConfig.recreateServiceCodesWithTooth.push(value);
          this.emitToParent.emit({ action: 'updateRecreateServiceCodesWithTooth', value: this.claimDetailPageConfig.recreateServiceCodesWithTooth });
        }
      } else {
        this.claimDetailPageConfig.recreateServiceCodesWithTooth.forEach((e: any, idx: any) => {
          if (e.name == value.name) {
            this.claimDetailPageConfig.recreateServiceCodesWithTooth.splice(idx, 1);
            this.emitToParent.emit({ action: 'updateRecreateServiceCodesWithTooth', value: this.claimDetailPageConfig.recreateServiceCodesWithTooth });
          }
        });
    ...
    else if (type === 'pendencyClients') {
      this.pendencyTeamConfig[filterProperty] = [];

      this.inputConfig[filterProperty].forEach((item: any) => {
        if (item.checked) {
          item.checked = true;
          this.pendencyTeamConfig[filterProperty].push(item);
        }
      })


      if (this.pendencyTeamConfig[filterProperty].length == this.inputConfig[filterProperty].length) {
        this.isAllSelected[filterProperty] = true;
        this._service.emitOnValueChange({ action: 'getSelectedClient', value: this.pendencyTeamConfig[filterProperty] });
      } else {
        this.isAllSelected[filterProperty] = false;
        this._service.emitOnValueChange({ action: 'getSelectedClient', value: this.pendencyTeamConfig[filterProperty] });
      }
    }
    else if (type === 'selectAllPendencyClients') {
      if (status) {
        this.pendencyTeamConfig[filterProperty] = [];
        this.inputConfig[filterProperty].forEach((item: any) => {
          item.checked = true;
