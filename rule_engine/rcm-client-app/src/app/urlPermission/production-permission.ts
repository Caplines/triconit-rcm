...
export const ProductionPermissionActivate: CanActivateFn = (
    ...
    let client: any = localStorage.getItem('selected_clientName');
    if (!ut) {
        router.navigate(['/']);
        return false;
    }
    if (ut != '-1' && (!(client != 'Smilepoint' && ut == 5) || client == 'Smilepoint')) {
...
export const canActivateChild: CanActivateChildFn = (route: ActivatedRouteSnapshot, state: RouterStateSnapshot) => ProductionPermissionActivate(route, state);