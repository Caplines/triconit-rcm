...
public interface RcmClaimRepository extends JpaRepository<RcmClaims, String> {
	...
	@Query(nativeQuery = true, value = ""
			...
			+ " rcau.first_name as assignedToFname,rcau.last_name as assignedToLname,Case When claims.rebilled_status Then 1 ELSE 0 End as rebilledStatus "
	...
	@Query(nativeQuery = true, value = " select off.name as officeName,claims.claim_uuid as uuid ,claims.claim_id as claimId,claims.patient_id as patientId,"
			...
			//+ " -- and rca1.active=false and rca1.current_team_id!=7 and "
			+" and rca1.id =( "
			+" select  min(fa.id) "
			+"     FROM rcm_claim_assignment fa "
			+"     WHERE fa.claim_id=claims.claim_uuid and fa.current_team_id!=7 and  fa.active=false "
			+"     LIMIT 1) "
			+ " left join rcm_user rcau on rcau.uuid=rca.assigned_to "
			+ " left join rcm_team lastteam on lastteam.id=claims.last_work_team_id "
			+ " left join rcm_insurance insurance on insurance.id=claims.prim_insurance_company_id "
	...
	@Query(nativeQuery = true, value = " select off.name as officeName,claims.claim_uuid as uuid ,claims.claim_id as claimId,claims.patient_id as patientId,"
			...
			+ " claims.next_follow_up_date as followUpDate,claims.balance_from_es_after_posting as dueBalance,claims.is_primary as claimTypeStatus,Case When claims.rebilled_status Then 1 ELSE 0 End as rebilledStatus "
			+ " from rcm_claims claims "
			+ " left join rcm_team team on team.id=claims.current_team_id "
			+ " inner join office off on off.uuid=claims.office_id "
	...
	@Query(nativeQuery = true, value = " select off.name as officeName,claims.claim_uuid as uuid ,claims.claim_id as claimId,claims.patient_id as patientId,"
			...
			+ " left join rcm_insurance secinsurance on secinsurance.id=claims.sec_insurance_company_id "
			+ " left join rcm_insurance_type secinsuranceT on secinsuranceT.id=secinsurance.insurance_type_id "
			+ "  where claims.current_team_id=:teamid  and off.company_id=:companyId and off.active is true and pending=true "
			+ " and claims.current_status<>"+Constants.CLAIM_CLOSED+" "
			+"  and claims.current_state="+Constants.CLAIM_ARCHIVE_PREFIX_CANBE_SUBMITED+" "
			+ " and (primary_status ="+Constants.Primary_Status_Primary+" or primary_status ="+Constants.Primary_Status_Primary_submit+"  ) order by ust-claimAge,primeSecSubmittedTotal asc ")
	List<FreshClaimDataDto> fetchUnBilledClaimByTeamAndClient(@Param("companyId") String companyId, @Param("teamid") int teamid);
	*/
	@Query(nativeQuery = true, value = " select off.name as officeName,claims.claim_uuid as uuid ,claims.claim_id as claimId,claims.patient_id as patientId,"
			+ " claims.dos as dos ,claims.patient_name as patientName,claims.attachment_count as attachmentCount, "
			+ " claims.claim_status_type_id as statusType,insurance.name as primaryInsurance "
			...
			+ " case when rca.pending_since is not null then rca.pending_since else claims.created_date end as pendingSince,claims.status_es as statusES,claims.status_es_updated as statusESUpdated,claims.next_action as nextAction,"
			+ " claims.next_follow_up_date as followUpDate,claims.balance_from_es_after_posting as dueBalance,claims.is_primary as claimTypeStatus, "
			+ " rcau.first_name as assignedToFname,rcau.last_name as assignedToLname,team.name as assignedToTeam,Case When claims.rebilled_status Then 1 ELSE 0 End as rebilledStatus "
			+ " from rcm_claims claims "
			+ " left join rcm_team team on team.id=claims.current_team_id "
			+ " inner join office off on off.uuid=claims.office_id  "
			+ " left join rcm_claim_assignment rca on rca.claim_id=claims.claim_uuid and rca.active=1  "
			...
			+ " left join rcm_insurance secinsurance on secinsurance.id=claims.sec_insurance_company_id "
			+ " left join rcm_insurance_type secinsuranceT on secinsuranceT.id=secinsurance.insurance_type_id "
			+ "  where off.company_id=:companyId and off.active is true and (pending=true or current_status=:status)"
			+ " and claims.current_status<>"+Constants.CLAIM_CLOSED+" "
			+"  and claims.current_state="+Constants.CLAIM_ARCHIVE_PREFIX_CANBE_SUBMITED+" "
			+ " and (primary_status ="+Constants.Primary_Status_Primary+" or primary_status ="+Constants.Primary_Status_Primary_submit+"  ) order by ust-claimAge,primeSecSubmittedTotal asc ")
	List<FreshClaimDataDto> fetchAllUnBilledClaimByClient(@Param("companyId") String companyId,@Param("status") int status);
	...
	@Query(nativeQuery = true, value = Constants.PENDENCY_REPEAT_FRESH_QUERY_ByTeamType +"  group by rca.claim_uuid, cmp.name, off.name, off.uuid, cl.claim_uuid having count(rca.claim_uuid) = 1 ")
	...
	@Query(nativeQuery = true, value = " SELECT off.name as officeName,off.uuid as  officeUuid,"
			...
			+ " us.uuid as assignedUser,us.first_name as fName,us.last_name  as lName,assig.team_id as assignTeamId,Case When cl.rebilled_status Then 1 ELSE 0 End as rebilledStatus  " + " FROM "
			+ "  office off left join rcm_claims  " + "  cl on off.uuid=cl.office_id "
			+ "  left join rcm_insurance_type inst on inst.id=cl.rcm_insurance_type  "
	...
	*/
	...
	//Patient Calling
		...
		@Modifying
		@Transactional
		@Query(value = "update rcm_claims set is_force_unassigned=true,force_unassigned_comment=:comment,"
