...
public interface RcmClaimRepository extends JpaRepository<RcmClaims, String> {
	...
	@Query(nativeQuery = true, value = " SELECT cmp.name as companyName,off.name as officeName,off.uuid as  officeUuid,"
			...
			+ "  where  off.company_id in (:companyIds) and off.active is true  group by off.uuid order by off.name asc ")
	...
	@Query(nativeQuery = true, value = " select claim_uuid uuid,cl.claim_id claimId,dos,patient_birth_date  patientDob,"+
			...
			" cl.treating_provider_from_claim_on_sheet treatingProviderFromSheet " +
