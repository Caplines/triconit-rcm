...
//	}
	...
	@GetMapping(value = "api/get-patient-payment-info/{claimUuid}/{withTeam}")
	@PreAuthorize("hasAnyRole('SUPER_ADMIN','TL','ASSO')")
	public ResponseEntity<?> getPatientPaymentInfo(@PathVariable("claimUuid") String claimUuid,
...
//	}
	...
	public ResponseEntity<?> getRequestRebillingCodes(@PathVariable("claimUuid") String claimUuid, Model model) {
		...
		if (partialHeader == null)
			return ResponseEntity.badRequest()
					.body(new GenericResponse(HttpStatus.BAD_REQUEST, MessageConstants.SOMETHING_WENT_WRONG, null));
		RebillingDto response = null;
		try {
			response = claimSection.fetchRequestRebillingServiceCodesInformation(partialHeader, claimUuid);
		} catch (Exception e) {
			e.printStackTrace();
			logger.error(e.getMessage());
			return ResponseEntity.badRequest().body(new GenericResponse(HttpStatus.INTERNAL_SERVER_ERROR, "", null));
	...
	public ResponseEntity<?> validateRecreateClaim(@RequestBody ValidateCreateClaimInformationDto dto, Model model) {
		...
		if (!StringUtils.isNoneBlank(dto.getCurrentClaimUuid())) {
			return ResponseEntity.badRequest()
					.body(new GenericResponse(HttpStatus.BAD_REQUEST, MessageConstants.EMPTY_RESOURCE, null));
		}
		try {
