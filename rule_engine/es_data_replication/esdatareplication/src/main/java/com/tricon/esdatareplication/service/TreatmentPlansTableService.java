...
public class TreatmentPlansTableService extends CommonTableService{
	...
	public ESTable pushDataFromLocalESToColudDB(BufferedWriter bw, Office office, ESTable es) {
		try {
			List<TreatmentPlans> pL = treatmentPlansRepository.findByMovedToCloud(DataStatus.StatusEnum.DATA_CLOUD_STATUS.NO);
			...
			((List<TreatmentPlansReplica>) repList).stream().map(TreatmentPlansReplica::getTreatmentPlanId).forEach(apptIdInES::add);
			// or
			// d2.forEach(a -> patIds.add(a.getPatientId()));
			List<TreatmentPlansReplica> inDB = treatmentPlansRepositoryRe.findByTreatmentPlanIdInAndOfficeId(apptIdInES,office.getUuid());
			...
			if (apptIdInDB.size() > 0) {
				apptIdInDB.forEach(id -> {
					TreatmentPlansReplica p = ((List<TreatmentPlansReplica>) repList).stream().filter(dp -> id.intValue()==dp.getTreatmentPlanId().intValue())
							.findAny().orElse(null);

					TreatmentPlansReplica old = inDB.stream().filter(ind -> id.intValue()==ind.getTreatmentPlanId().intValue()).findAny()
							.orElse(null);
					if (p!=null && old!=null) {
						if (old!=null) {
							p.setId(old.getId());
	...
	public void saveDataToLocalDB(BufferedWriter bw,List<?> data, boolean checkExisting) {
		...
		if (!checkExisting) {
			...
			if (apptIdInES.size() > 0) {
				...
				apptIdInES.forEach(id -> {
					l.add(((List<TreatmentPlans>) data).stream().filter(p -> id.equals(p.getTreatmentPlanId()))
							.findAny().orElse(null));
				});
				if (l.size() >0) treatmentPlansRepository.saveAll(l);
			}
