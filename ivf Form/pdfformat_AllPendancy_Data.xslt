...
<xsl:output method="xml" indent="yes" encoding="UTF-8"  version="1.0"  />
    	<xsl:template match="/allPendancyDownloadDto">
         ...
         <tr>
             <td class="bgWhite">
                 ...
                 <xsl:variable name="currentTeam" select="currentTeamName" />
                  ...
                  <xsl:if test="tabSwitch='withoutDos'"> 
                         <xsl:for-each select="data/data/counts1/entry">                      
                           ...
                           <xsl:variable name="officeName" select="../../officeName" />
                                   <xsl:if test="$currentTeam!= $currentKey">
                                        <tr class="whiteBg">            
                                           ...
                                           <td><xsl:value-of select="$currentKey" /></td>
                                           <td><xsl:value-of select="$currentValue" /></td>
                                        </tr>
                                    </xsl:if>
                          </xsl:for-each>
                           <tr>
                <td colspan="3" class="totalsRow">
                  <xsl:text>Totals:</xsl:text>
                  <xsl:for-each select="totalCount">
                     <xsl:if test="$cTeamId!= teamId">
                    <xsl:variable name="counts" select="count" />
                    <xsl:variable name="teamNames" select="teamName" />
                    <xsl:value-of select="concat(' ', $teamNames, '=' $counts)" />
                     <xsl:if test="position() != last()">
                            <xsl:text>, </xsl:text>
                                ...
                                <xsl:if test="tabSwitch='withDOS'"> 
                         <xsl:for-each select="data/data/dates1/entry">                      
                         <xsl:variable name="currentKey" select="key" />
                         <xsl:variable name="currentValue" select="value" />
                           <xsl:variable name="officeName" select="../../officeName" />
                                   <xsl:if test="$currentTeam!= $currentKey">
                                        <tr class="whiteBg">            
                                           ...
                                           <td>
                                            ...
                                            <xsl:variable name="month" select="substring($currentValue, 6, 2)" />
                       <xsl:variable name="day" select="substring($currentValue, 9, 2)" />
                       <xsl:variable name="year" select="substring($currentValue, 1, 4)" />
                 ...
                 <xsl:if test="tabSwitch='withDOP'"> 
                         <xsl:for-each select="data/data/datesPending/entry">                      
                         <xsl:variable name="currentKey" select="key" />
                         <xsl:variable name="currentValue" select="value" />
                           <xsl:variable name="officeName" select="../../officeName" />
                                   <xsl:if test="$currentTeam!= $currentKey">
